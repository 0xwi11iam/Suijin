...
# ── 7/8 launcher + PATH ────────────────────────────────────────────────
step "installing 'suijin' + 'suijind' launchers"
mkdir -p "$BIN_DIR"
...
# suijind — the daemon by another name. Same interpreter, same repo, no
# root, no service, no ports: it runs an engagement as a child process in
# YOUR session. (Bin dir is inside $HOME by default — no sudo anywhere.)
DAEMON_LAUNCHER="$BIN_DIR/suijind"
cat > "$DAEMON_LAUNCHER" <<EOF
#!/usr/bin/env bash
# Suijin daemon launcher — generated by install.sh
# No admin rights required: a detached child process in your own session.
exec "$VENV/bin/python" "$REPO_DIR/suijin/modules/console/lib/cli.py" suijind "\$@"
EOF
chmod +x "$DAEMON_LAUNCHER"
ok "daemon launcher at $DAEMON_LAUNCHER"
