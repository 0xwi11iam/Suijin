    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, (fn, params) in _load_tools().items():
            ...
            ctx.register_tool(
                tool_name,
                _bridge,
                description="Real payload generation: reverse shells (5 context types), encoder chains (base64/gzip/hex/URI), and stager builders. All output is runnable code.",
                owner="payloadforge",
                params=params,
            )
            bridged += 1
        ctx.journal.append("payloadforge", f"{bridged} tool(s) registered")
