...
TOOL_REGISTRY = {
    ...
    "js_bundle_analyze": {
        "purpose": "Mine a JS bundle for the app's real attack surface",
        "when_to_use": "SPAs (Vite/React/Next on Vercel/Netlify): download the main bundle ONCE and get routes (/api/*, /admin), absolute URLs, leaked secrets (API keys, JWTs, firebase/supabase configs), auth providers, lazy chunks, sourcemap refs. Replaces hand-rolled curl+grep chains.",
        "args_format": '"url": "https://target.com/assets/index-abc123.js"',
        "description": "**js_bundle_analyze** — routes/URLs/secrets/providers/chunks/sourcemaps from one bundle.",
    },
    "google_key_probe": {
        "purpose": "Test a leaked Google API key against common services",
        "when_to_use": "After js_bundle_analyze finds an AIza... key — read-only probes report which services accept it (maps/translate/youtube) and whether it is referrer-restricted.",
        "args_format": '"key": "AIza..."',
        "description": "**google_key_probe** — per-service ACTIVE / restricted / not-enabled verdicts.",
    },
    "source_map_probe": {
        "purpose": "Check a JS/CSS asset for an exposed source map",
        "when_to_use": "When a bundle references a .map or you suspect deployed maps — exposed source maps recover the original source tree (API logic, auth, keys).",
        "args_format": '"url": "https://target.com/assets/index-abc123.js"',
