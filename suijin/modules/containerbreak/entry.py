    ...
    def start(self, ctx) -> None:
        ...
        for tool_name, (fn, params) in _load_tools().items():
            ...
            ctx.register_tool(
                tool_name,
                _bridge,
                description="Container escape analysis: inspect exposed Docker API configs (privileged, capabilities, mounts) and generate concrete escape commands from real config analysis.",
                owner="containerbreak",
                params=params,
            )
            bridged += 1
        ctx.journal.append("containerbreak", f"{bridged} tool(s) registered")
